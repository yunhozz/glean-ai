...
`config/interests.yaml`에서 키워드와 subreddit을 수정합니다. Reddit은 Client ID/Secret과 고유 User-Agent가 필요하고 Threads는 `threads_basic`, `threads_keyword_search` 권한을 승인받은 장기 User Access Token이 필요합니다. 실제 secret은 `.env` 또는 GitHub Secrets에만 둡니다.
